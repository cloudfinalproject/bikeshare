...
class BikeDetailsViewController: UIViewController {
            ...
            if let httpResponse = response as? NSHTTPURLResponse {
                if(httpResponse.statusCode == 200){
                    ...
                    do{
                        ...
                        self.lng = jsonDict["results"]!![0]["geometry"]!!["location"]!!["lon"] as? Float
    ...
    @IBAction func editBike(sender: AnyObject) {
        ...
        //let coor = get_coordinate(address + "," + city + ", " + state)
        
        
        
        let request = NSMutableURLRequest(URL: NSURL(string: serverDomain + "/editBike/" + "\(bid)")!)
        request.HTTPMethod = "POST"
        let payload = "model=\(bikeModel)&address=\(address)&city=\(city)&state=\(state)&country=\(country)&postcode=\(postcode)&price=\(price)&available=\(availability)&details=\(details)&lat=\(lat)&lon=\(lng)"
